    ...
    public void run(IstarModel IS, ACModel AC) {
        ...
        for (int i = 0; i < IS.getRelations().size(); i++) {
            if (IS.getRelations().get(i).getType().equals("Goal")
                ...
                if (
                        (
                                        typeSource.equals("NodeGoal")
                                        || typeSource.equals("NodeQuality")
                                        || typeSource.equals("NodeResource")
                                        || typeSource.equals("NodeTask")
                        )
                                &&
                                (
                    ...
                    String sr1 = typeSource;
                    typeSource = t.typeofInActors(IS.getRelations().get(i).getOrigin(), IS.getNodes());
                    //Origen
                    ...
                    AC.getCommunicativeEvents().add(new CommunicativeEvent(
                            IS.getRelations().get(i).getId()+"I1",
                            "",
                            sr1.substring(3),
                            "",
                            "",
