    ...
    public void run(IstarModel IS, ACModel AC) {
        ...
        for(int i=0;i<IS.getNodes().size();i++){
            if((IS.getNodes().get(i).getType().equals("Actor"))||(IS.getNodes().get(i).getType().equals("Role"))||(IS.getNodes().get(i).getType().equals("Agent"))){
                ...
                AC.getCommunicativeInteractions().add(new CommunicativeInteraction("A2", "", "To register Information of "+IS.getNodes().get(i).getName(), "", "A0"+IS.getNodes().get(i).getName(), "A1", new MessageStructure("RegisterInformation", "")));
            }
        }
    }
}
